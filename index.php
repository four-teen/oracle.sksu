...
<button id="chatbotBtn"
  ...
  <img src="assets/img/cbot.gif" alt="Animated chatbot icon" style="max-width:100%; max-height:100%;">
</button>
    ...
    <div class="modal-content shadow rounded-4">
      <div class="modal-header border-0 pb-0">
        <h5 class="modal-title" id="chatbotModalLabel">
          <img src="assets/img/dbot.gif" alt="Oracleu AI" width="28" height="28" class="me-2">
          Oracle AI
        </h5>
      ...
      <div class="modal-body pt-2 pb-0">
        <div id="chatbox" class="d-flex flex-column" style="max-height: 50vh; overflow-y: auto;">
          <div class="ls-msg bot">
            <div class="bubble">
              Hello! I'm your virtual guide for Oracle. Please enter you research title!
            </div>
          </div>
        </div>
      </div>
...
chatForm.addEventListener('submit', async (e)=>{
  ...
  typing.textContent = 'Oracle AI is searching titles…';
  chatbox.appendChild(typing);

  try{
    const res = await fetch('ai_match_titles.php', {
      method: 'POST',
    ...
    if (data.error){
      addMessage('bot', 'AI matching error. (' + data.error + ')');
      console.error(data); return;
    }

    const list = (data.top_matches || []).map(m =>
      `<li><strong>${m.title}</strong><br><small>Reason: ${m.reason}</small></li>`
    ).join('');

    addMessage('bot',
      `<div><strong>Top similar titles</strong>
        <ul>${list || '<li>No close matches found.</li>'}</ul>
      </div>`
    );
  }catch(err){
    typing.remove();
    addMessage('bot', 'Server error. Try ulit.');
    console.error(err);
  }
});
</script>
